...
[Pulling Repo Update](#pulling-repo-update)
...
Place your variables in .env then:


Change ```settings.py``` again:
...
### Upload sqlite db to instance:
Exit ec2 instance:
```
exit
```

COPY YOUR DB FILE TO YOUR KEY PAIR DIRECTORY, THEN:

In Git Bash navigate to AWS Key directory (you should already be here)

Run the command:
```
scp -i aws-keypair.pem db.sqlite3 ubuntu@ec2-54-241-207-228.us-west-1.compute.amazonaws.com:/home/ubuntu/hawaiitraveltips/
```

* Replace `aws-keypair.pem` with key pair
* `db.sqlite3` is actually the path to the db but you should have copied it into the directory so this is the path
* Replace `ec2-18-222-108-201.us-east-2.compute.amazonaws.com` with public DNS
* DNS uses `-` between numbers whereas the IPv4 uses `.`


### Connect back to instance and migrate db
change directory into your project to the directory with ```manage.py```
```
